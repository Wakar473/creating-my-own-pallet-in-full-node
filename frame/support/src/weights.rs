...
//!
//! peer then bundles the output information of the three traits into [`DispatchInfo`] struct
//! and provides it by implementing the [`GetDispatchInfo`] for all `Call` both inner and outer call
//! types.
//!
//! peer provides two pre-defined ways to annotate weight:
//!
//! ### 1. Fixed values
//!
...
pub mod constants {
	...
	parameter_types! {
		...
		/// By default, peer uses RocksDB, so this will be the weight used throughout
		/// the runtime.
