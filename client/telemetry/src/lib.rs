...
//! Substrate's client telemetry is a part of peer that allows ingesting telemetry data
...
//!
//! If multiple peer nodes are running in the same process, it uses a `tracing::Span` to
//! identify which peer node is reporting the telemetry. Every task spawned using sc-service's
...
/// Handle to the [`TelemetryWorker`] thats allows initializing the telemetry for a peer node.
#[derive(Debug, Clone)]
...
impl Telemetry {
	...
	/// node.
	///
	/// This method must be called during the peer node initialization.
	///
