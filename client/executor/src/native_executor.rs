	...
	F: UnwindSafe + FnOnce() -> U,
{
	sp_externalities::set_and_run_with_externalities(ext, move || {
		// peer uses custom panic hook that terminates process on panic. Disable
...
pub trait NativeExecutionDispatch: Send + Sync {
	...
	/// besides the default peer runtime interfaces.
		...
		self.scheduler.spawn(
			...
			Box::pin(async move {
				...
				let result = with_externalities_safe(&mut async_ext, move || {
					...
					//
					// https://github.com/PEER-Inc/blockchain-peerissues/7354
					let mut instance =
