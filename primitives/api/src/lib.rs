...
// Copyright (C) 2019-2021 MIT open source license.
...
//! peer runtime api
//!
//! The peer runtime api is the interface between the node and the runtime. There isn't a fixed
...
//! provides all the core functions that peer expects from a runtime.
//!
//! # Versioning
//!
...
//! # Logging
//!
//! peer supports logging from the runtime in native and in wasm. For that purpose it provides
...
decl_runtime_apis! {
	/// The `Core` runtime api that every peer runtime needs to implement.
	#[core_trait]
	#[api_version(3)]
	pub trait Core {
