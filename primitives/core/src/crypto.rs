...
pub trait Pair: CryptoType + Sized + Clone + Send + Sync + 'static {
	...
	/// as a password.
	/// - If `s` begins with a `/` character it is prefixed with the peer public `DEV_PHRASE`
	///   and
...
pub mod key_types {
	...
	/// Key type for controlling an account in a peer runtime, built-in. Identified as `acco`.
